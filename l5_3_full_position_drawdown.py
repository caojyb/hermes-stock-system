...
"""
L5-3 满仓回撤测试
与 L5-2 逻辑一致，仅把初始仓位从 20% 改为 100%（10 只等权，每只 10%）
连续 5 个交易日，每天 3 只触发 -8% 止损
"""
import sqlite3
...
POSITION_PCT = 1.0           # 总仓位 100%
STOP_LOSS = 0.08
SLIPPAGE = 0.05
PORTFOLIO_DRAWDOWN_LIMIT = 0.15
COOLING_DAYS = 3
EXEMPTION_THRESHOLD = 0.04
...
def init_portfolio(candidates):
    ...
    con.commit()
    con.close()
    print(f"已初始化前 10 只候选为持仓（每只目标仓位 10%）：")
    for code, name, sector, _ in candidates[:10]:
...
def check_portfolio_drawdown():
    ...
    cur.execute("""
        ...
        WHERE date NOT LIKE 'cooling_%'
        ORDER BY rowid DESC
    """)
    rows = cur.fetchall()
    con.close()
    if not rows:
        return False, 0.0, TOTAL_CAPITAL
    current_value = rows[0][1]
    recent_high = max(r[1] for r in rows)
    drawdown = (recent_high - current_value) / recent_high
    return drawdown >= PORTFOLIO_DRAWDOWN_LIMIT, drawdown, recent_high


def is_cooling_active():
    con = sqlite3.connect(SIM_DB)
    cur = con.cursor()
    cur.execute("""
...
def check_cooling_exemption(csi2000_changes):
    if len(csi2000_changes) < 2:
        return False
    dates = sorted(csi2000_changes.keys())[-2:]
    return all(csi2000_changes[x] > EXEMPTION_THRESHOLD for x in dates)
...
def run_simulation():
    ...
    holding_codes = init_portfolio(candidates)
    ...
    days = [
        {"day": 1, "stop_indices": [0, 1, 2], "others_pct": -10.0, "csi2000": -1.0},
        {"day": 2, "stop_indices": [0, 1, 2], "others_pct": -12.0, "csi2000": -1.0},
        {"day": 3, "stop_indices": [0, 1],      "others_pct": -15.0, "csi2000": +4.2},
        {"day": 4, "stop_indices": [0],          "others_pct": -20.0, "csi2000": +4.6},
        {"day": 5, "stop_indices": [],           "others_pct": -10.0, "csi2000": +2.0},
        {"day": 6, "stop_indices": [],           "others_pct": +5.0, "csi2000": +1.5},
    ]
    ...
    for d in days:
        ...
        stop_set = set(d['stop_indices'])
        stop_list = [h for i, h in enumerate(holdings) if i in stop_set]
        others = [h for i, h in enumerate(holdings) if i not in stop_set]
        ...
        csi2000_changes[f"d{d['day']}"] = d['csi2000']

        trim_triggered = False
        cooling = is_cooling_active()
        exemption = False

        should_trim, dd_val, _ = check_portfolio_drawdown()
        if should_trim and trim_triggered_day is None:
            trim_triggered = True
        ...
        if cooling and not trim_triggered:
            exemption = check_cooling_exemption(csi2000_changes)
            if exemption:
                con = sqlite3.connect(SIM_DB)
                cur = con.cursor()
    ...
    return results, trim_triggered_day, day_stop_details, holding_codes


def print_results(results, trim_day, details, holding_codes):
    print("=" * 120)
    print("L5-3 满仓回撤测试结果")
    print("=" * 120)
    print(f"候选池：23 只（double_up_scores 最新一期）")
    print(f"初始持仓：前 10 只，每只 10%，总仓位 100%")
    ...
    final = results[-1]
    max_drawdown = max(r['drawdown_pct'] for r in results)
    print("最终状态（第 5 天收盘后）：")
    ...
    if trim_day:
        print(f"  15% 减仓在第 {trim_day} 天触发")
        print(f"  减仓后组合仓位降至 50%，进入冷却期 {COOLING_DAYS} 天")
        print("  冷却期内系统行为：不开新仓，现有持仓继续按正常流程监控止盈止损")
        if any(r['exemption'] for r in results):
            print("  冷却期已被中证2000连续反弹豁免")
        else:
            print("  冷却期未被豁免，需自然结束")
    else:
        print("  15% 减仓线未触发")
...
if __name__ == "__main__":
    res, trim_day, details, codes = run_simulation()
    print_results(res, trim_day, details, codes)
